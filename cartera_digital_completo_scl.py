...
cols_cost = ['OPERATION_NUMBER', 'OPERATION_NAME', 'OPERATION_TYPE_NAME', 'OPERATION_MODALITY', 'DEPARTMENT', 'DIVISION', 'REGION', 'COUNTRY', 'STATUS', 'STAGE', 'TAXONOMY', 'EXEC_STS', 'APPROVAL_DATE', 'APPROVAL_YEAR',
        'APPROVAL_AMOUNT', 'CURRENT_EXPIRATION_DATE', 'RELATED_OPER', 'RELATION_TYPE', 'OPERATION_TYPE', 'OBJECTIVE_EN', 'OBJECTIVE_ES', 'COMPONENT_NAME', 'OUTPUT_NAME','OUTPUT_DESCRIPTION',  'OUTPUT_UOM','IS_COST', 'OUTPUT_COST']
Metadatos_tot = pd.DataFrame(columns=cols_cost)
result_cost = ibm_db.fetch_both(stmt_cost)
while(result_cost):
    Metadatos_tot = Metadatos_tot.append(result_cost, ignore_index=True)
    result_cost = ibm_db.fetch_both(stmt_cost)

Metadatos_tot = Metadatos_tot.iloc[:, 0:27]
Metadatos_tot.shape #Visualizando resultado

###### Datos para generar resultados #######

Metadatos = Metadatos_tot[(Metadatos_tot.IS_COST=="1") | (Metadatos_tot.IS_COST.isnull())]
Resultados = Metadatos_tot[(Metadatos_tot.IS_COST=="0")]
Resultados['OUTPUT_FI']=Resultados['OUTPUT_COST']
Resultatos = Resultados[['OPERATION_NUMBER','COMPONENT_NAME', 'OUTPUT_NAME', 'OUTPUT_FI', 'OUTPUT_UOM']]
...
Producto1 = Producto1.merge(Metadatos[['OUTPUT_COST', 'IS_COST', 'OPERATION_NUMBER','OUTPUT_NAME', 'OUTPUT_UOM', 'DIVISION', 'COMPONENT_NAME']], on = ['OPERATION_NUMBER','OUTPUT_NAME', 'COMPONENT_NAME'], how = 'left')
...
Producto1 = Producto1.merge(Resultados[['OPERATION_NUMBER', 'COMPONENT_NAME', 'OUTPUT_NAME', 'OUTPUT_FI']],  on = ['OPERATION_NUMBER', 'COMPONENT_NAME', 'OUTPUT_NAME'], how = 'left')
Producto1 = Producto1.drop_duplicates()

# Group digital cost by operation ONLY DIGITALS 
Costos_op = Producto1.groupby(['OPERATION_NUMBER', 'COMPONENT_NAME','RESULT_OUTPUT_NAME', 'IS_COST'], as_index=False)['OUTPUT_COST'].sum() # totales costos por output 
Costos_tot = Costos_op.groupby(['OPERATION_NUMBER', 'IS_COST'], as_index=False)['OUTPUT_COST'].sum() # totales por operación
...
Final['DUMMY_DIGITAL']=np.where((Final['RESULT_OPERATION_NAME']=='DIGITAL') | (Final['RESULT_OBJETIVO']=='DIGITAL') | (Final['DIGITAL_COMP']>0) | (Final['DIGITAL_OUT']>0.1),1,
                                       np.where((Final['DIGITAL_OUT']>0.045) & (Final['OUTPUT_COST']>0), 1,
                                                np.where((Final['OPERATION_NUMBER']=='BR-L1526') |(Final['OPERATION_NUMBER']=='CO-L1229') |(Final['OPERATION_NUMBER']=='EC-L1155') |(Final['OPERATION_NUMBER']=='ME-L1162')| (Final['OPERATION_NUMBER']=='UR-L1176'),1,0)))
...
Final=Final[['OPERATION_NUMBER','DUMMY_DIGITAL','DUMMY_OBJETIVO_DIG','DUMMY_OUTPUT_DIG','DUMMY_TITULO', 'DUMMY_COMPONENTE', 'OUTPUT_COST']]
...
checklist = pd.read_excel(path_cl+"/Triage_digital.xlsx", sheet_name = "Sheet1") # leer checklist completa 
...
checklist_port = checklist_tidy.merge(Bas, on = 'OPERATION_NUMBER', how = 'left') # Juntar con total para ver operaciones en cartera
today = time.strftime("%Y-%m-%d")
checklist_port2 = checklist_port.loc[(checklist_port['APPROVAL_DATE']<today)]
checklist_port2 = checklist_port2[['OPERATION_NUMBER','RELATED_OPER','RELATION_TYPE','EXEC_STS','OPERATION_TYPE','OPERATION_TYPE_NAME',
           'OPERATION_MODALITY','TAXONOMY','STATUS','REGION','COUNTRY','DEPARTMENT','DIVISION','APPROVAL_DATE',
           'APPROVAL_AMOUNT','CURRENT_EXPIRATION_DATE', 'INFO','INFRA','GOB','CULTURA','DUMMY_DIGITAL_CL','DUMMY_OBJETIVO_DIG', 
           'DUMMY_OUTPUT_DIG', 'DIGITAL_TRANS']]
...
Bas = Bas.merge(Costos_tot[['OPERATION_NUMBER', 'TOTAL_COST']], on =['OPERATION_NUMBER'] , how = 'left')
Bas['OUTPUT_DIG_COST']= Bas['OUTPUT_COST']/Bas['TOTAL_COST']

# meter operaciones de edu manualmente aquí 
