...
"con este script se llaman las operaciones del pipeline para el triage digital y se corre el diccionario para clasificarla como digital o no\
    también se lee la información reecolectada en el excel del checklist para determinar si una operación fue catalogada como digital por los especialistas.\
        Por ahora hay muchas operaciones que no aparecen clasificadas, ni por el diccionario, ni checklist se dejan como no se tiene información suficiente."
...
#Componentes['DIGITAL_COMP']=Componentes['DIGITAL']/(Componentes['DIGITAL']+Componentes['NO DIGITAL']) por ahora en los que se puede reconocer no hay comp digital entonces se quita esta línea, probar cada que se corra.
...
Final['DUMMY_DIGITAL']=np.where((Final['RESULT_OPERATION_NAME']=='DIGITAL') | (Final['RESULT_OBJETIVO']=='DIGITAL'),1,np.where(((Final['RESULT_OPERATION_NAME']=='DIGITAL') | (Final['RESULT_OBJETIVO']=='DIGITAL')) |((Final['DIGITAL_OUT']>0)),1,0))
...
Bas=Metadatos[['OPERATION_NUMBER','RELATED_OPER','OPERATION_TYPE','OPERATION_TYPE_NAME',
...
########EXPORTAR ARCHIVOS#############
with pd.ExcelWriter(path+"/Outputs/output-pipe-dicc.xlsx") as writer:
